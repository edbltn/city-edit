-- Bicycle profile — FLAT (shortest-legal-path) variant for import corrections.
--
-- Extracted verbatim from osrm/osrm-backend:v5.25.0 /opt/bicycle.lua (the same
-- pinned image the foot dataset builds with), then patched so a route answers
-- one question only: "could a bike have LEGALLY RIDDEN this trip, and along
-- which stretches?" Used by scripts/build_bike_osrm.sh + server/counter_lyft.py
-- to subtract imported bikeshare votes wherever the ridden street would have
-- carried a bike anyway, leaving only pedestrian-only / counter-one-way
-- stretches upvoted. Deltas from stock:
--
--   1. weight_name 'duration' → 'distance': route choice is the shortest legal
--      path, not the fastest — so between via-points sampled along the ride's
--      voted path the router hugs that path wherever riding it is legal,
--      instead of detouring to faster parallel streets.
--   2. bike_push_handler gutted: stock converts foot-only ways (footway/
--      pedestrian/steps, bicycle=dismount) into slow pushing_bike edges.
--      Pushing is walking — exactly the signal we must NOT subtract — so here
--      those ways stay inaccessible and can never appear in a route.
--   3. use_public_transport=false, route_speeds/platform_speeds emptied: no
--      ferries, trains, or station platforms; trips cross rivers on bridges
--      they could ride, or detour.
...
function bike_push_handler(profile,way,result,data)
  -- FLAT variant: never fall back to pushing the bike. A stretch you can only
  -- walk (footway/pedestrian/steps, or bicycle=dismount) must stay out of the
  -- graph so it can never be subtracted from the imported votes.
