-- Foot profile
--
-- Pinned verbatim from osrm-backend v5.25.0 profiles/foot.lua. We ship our own
-- copy (instead of relying on /opt/foot.lua in the base image) so the routable
-- way/access rules are reviewable and versioned in-repo.
--
-- KEEP IN SYNC with server/foot_profile.py, which mirrors the `speeds`/access
-- tables below to build the votable topology graph as a superset of what this
-- profile routes. If you change the routable categories here, update that file.
