#!/usr/bin/env python3
"""Generate the HTML changelog report for the layer-anchor failsafe work.
...
Run from repo root:
  ...
  python changelog/build_anchor_failsafe_report.py
Reads changelog/changes.diff, writes changelog/2026-07-15-layer-anchor-failsafe.html
"""
import html
import os
import re
...
OUT_PATH = os.path.join(HERE, "2026-07-15-layer-anchor-failsafe.html")

DATE = "2026-07-15"
TITLE = "Layer-anchor failsafes — the map can no longer stay misaligned"
...
SECTIONS = [
    {
        "id": "anchor",
        "tag": "Frontend",
        "title": "1 · Transient layer misalignment — and why it could persist",
        "symptom": (
            "Screenshots at two zooms showed the white route/highlight layer offset from the "
            "block heat + base map underneath, with a different offset at each zoom. The drift "
            "self-healed on the next interaction — the signature of a <em>stale frame</em>, not "
            "bad projection math."
        ),
        "cause": [
            "Two renderers share the screen: Leaflet (SVG routes, pins, the highlight canvases) "
            "and MapLibre GL (base map + block heat). Both “ride” Leaflet's animated zoom with a "
            "CSS transform hack, then re-anchor afterwards — and both re-anchors hang off a "
            "fragile chain: MapLibre's camera is only re-synced by Leaflet's final "
            "<code>move</code> event, and the canvas repaint is scheduled on a bare "
            "<code>requestAnimationFrame</code>.",
            "Any dropped link leaves a layer painted for the OLD camera: a <code>zoomend</code> "
            "that arrives without a final <code>move</code>, an rAF that never fires (rAF "
            "freezes entirely in hidden/occluded windows — a known trap on this project), or a "
            "repaint that lands <em>mid-animation</em> and bakes inconsistent transforms into "
            "the bitmap (WS deltas and route-highlight events can trigger paints at any time).",
            "On block maps the exposure was worse: <code>drawStateRef</code> — the record of "
            "which camera the canvas bitmap was painted at — was only ever written on the "
            "no-blocks code path, so on block maps (exactly the maps in the screenshots) the "
            "zoom-anim ride and any drift detection flew blind.",
        ],
        "fixes": [
            "<strong>Never paint mid-zoom.</strong> <code>redraw()</code> and "
            "<code>redrawHoverHighlight()</code> bail while a zoom is in flight — a paint "
            "against a mid-animation pane transform lands misaligned and stays that way if the "
            "zoomend repaint drops. zoomend always schedules a redraw, so deferring loses "
            "nothing.",
            "<strong>Always record the painted camera.</strong> <code>drawStateRef</code> is "
            "now stamped at the top of every paint, before any early return — block maps "
            "included — so the ride and the failsafe below always know the bitmap's true anchor.",
            "<strong>Timer backstop for the rAF repaint.</strong> <code>scheduleRedraw()</code> "
            "arms a 200&nbsp;ms <code>setTimeout</code> alongside the rAF; whichever fires "
            "first wins and cancels the other. Timers are throttled but still run in hidden "
            "windows — rAF does not.",
            "<strong>Drift check on every move frame + visibility restore.</strong> "
            "<code>ensureAnchored()</code> compares the bitmap's painted zoom to the map's "
            "zoom (a number compare); on mismatch outside a zoom it logs "
            "<code>[topo] anchor drift…</code> and repaints <em>synchronously</em> — not via "
            "the rAF path that already failed once.",
            "<strong>MapLibre <code>reconcile()</code>.</strong> On <code>zoomend</code>, "
            "<code>moveend</code>, and visibility restore: clear any leftover ride transform, "
            "re-jump the camera <em>unconditionally</em> (no longer trusting that a final "
            "<code>move</code> preceded zoomend), and <code>triggerRepaint()</code> to force "
            "one GL frame. Each step is a cheap no-op when nothing drifted.",
        ],
        "files": [
            "client-react/src/components/GraphLayer/GraphLayer.tsx — mid-zoom paint guards, unconditional drawStateRef, rAF timer backstop, ensureAnchored drift check",
            "client-react/src/components/MapLibreBackground/MapLibreBackground.tsx — reconcile() on zoomend / moveend / visibilitychange",
        ],
    },
]

VERIFY = [
    "Frontend: <code>tsc --noEmit</code> — clean.",
    "Frontend: <code>vitest run</code> — 244 passed, 1 skipped.",
    "Dev stack: Vite (:3000) + Flask (:5001) serving; Vite confirmed serving the new <code>reconcile</code> code.",
]

CHECKLIST = [
    "Open <code>http://localhost:3000/m/nyc-walkways?tab=anchor</code>, zoom with wheel + the +/− control + pinch: the heat/highlights should track the base map through and after every zoom.",
    "Select a route proposal (white corridor), then zoom several levels quickly, interrupting animations mid-flight — the white corridor should land exactly on its blocks every time.",
    "Hide the window mid-zoom (Cmd-H or fully occlude it), wait 2s, bring it back — layers should be aligned on return (the visibilitychange reconcile), and any repair logs as <code>[topo] anchor drift</code> in a debug tab.",
    "While a route highlight is up, cast a vote from a second tab so a WS delta arrives, then zoom — no misaligned white corridor should ever persist after the zoom settles.",
]
...
FILE_CONTEXT = {
    "client-react/src/components/GraphLayer/GraphLayer.tsx": {
        ...
        "file": ("GraphLayer.tsx", "~5000 LOC — load topology+votes, paint the heat/highlight canvases, hit-test, render proposals"),
        "outline": [
            ("Module helpers", "decodeTopologyBin, buildEdgeIndex, votesMatchTopology, buildNodeAdj", False),
            ("Component state & refs", "canvas / topology refs, drawStateRef, NEW redrawFallbackRef", True),
            ("Canvas init", "create heat + hover canvases; cleanup now cancels pending rAF/timer", True),
            ("Topology + vote loading", "version → IndexedDB cache → network fetch → paint", False),
            ("redrawHoverHighlight", "white corridor/edge/node rings — now guarded against mid-zoom paints", True),
            ("redraw — heat passes", "mid-zoom guard + drawStateRef stamped before every early return", True),
            ("scheduleRedraw", "rAF fast path + NEW 200ms timer backstop (rAF freezes when hidden)", True),
            ("Map event handlers (pan / zoom)", "zoomanim ride; NEW ensureAnchored drift check on move + visibilitychange", True),
            ("Hover / hit-testing · indicator markers", "unchanged", False),
        ],
        "blocks": [
            "redrawFallbackRef — timer backstop handle next to redrawTimeoutRef",
            "canvas init cleanup — cancel pending rAF + timer on unmount",
            "redrawHoverHighlight — bail while isZoomingRef (mid-zoom paints land misaligned)",
            "redraw — same mid-zoom guard; drawStateRef stamped BEFORE the block-map early return",
            "redraw — old late drawStateRef assignment removed (now redundant)",
            "scheduleRedraw — rAF + 200ms setTimeout race; first to fire cancels the other",
            "ensureAnchored — painted-zoom vs map-zoom compare on move/visibilitychange → synchronous repaint + dwarn",
        ],
    },
    "client-react/src/components/MapLibreBackground/MapLibreBackground.tsx": {
        "on": ["React / Leaflet client"],
        "module": ("React client · components/MapLibreBackground", "the MapLibre GL base map + block heat, camera-synced from Leaflet"),
        "file": ("MapLibreBackground.tsx", "~540 LOC — GL style, camera sync, block feature-state heat"),
        "outline": [
            ("Style build", "carto raster + blocks PMTiles + heat/select paint", False),
            ("Map init / block resolver", "maplibregl.Map, blockAtResolver", False),
            ("Camera sync effect", "move→jumpTo, zoomanim ride; zoomend/moveend/visibility → NEW reconcile()", True),
            ("Block heat feature-state", "diff-applied heat + selection", False),
        ],
        "blocks": [
            "reconcile() — clear leftover ride transform, unconditional syncCamera, triggerRepaint; skips while _animatingZoom",
            "handleZoomEnd replaced — zoomend no longer trusts a preceding final move event",
            "moveend + document visibilitychange wired to reconcile (with cleanup)",
        ],
    },
}
...
<div class="wrap">
  ...
  <p class="lede">A screenshot pair showed the white route layer sitting offset from the block heat —
  differently at each zoom — then healing on its own. The projection math was fine; the re-anchor
  chain after zoom animations was fragile. Five failsafes now make every layer converge back onto
  the map no matter which link drops.</p>
  ...
  <footer>
    Generated from <code>changelog/changes.diff</code> by <code>changelog/build_anchor_failsafe_report.py</code>.
  </footer>
</div>
</body>
</html>
