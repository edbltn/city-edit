#!/usr/bin/env python3
"""Generate the HTML changelog report for the light-mode pin theme fix.
...
Run from repo root:
  ...
  python changelog/build_pin_theme_report.py
Reads changelog/changes.diff, writes changelog/2026-07-22-light-pin-theme-colors.html
"""
import html
import os
import re
...
OUT_PATH = os.path.join(HERE, "2026-07-22-light-pin-theme-colors.html")

DATE = "2026-07-22"
TITLE = "Light-mode pins wear the theme — accent-anchored glow + themed selection"
...
SECTIONS = [
    {
        "id": "pins",
        "tag": "Frontend",
        "title": "1 · Light-mode top-proposal pins read grey — and selected pins read blank",
        "symptom": (
            "On light map styles (terracotta / trees / plum) the top-proposal pin glow rings "
            "rendered as shades of grey, tan and near-black — never the map's identity color. "
            "A selected pin was worse: black double ring on a white fill, no visible theme at all."
        ),
        "cause": [
            "Pins sample their glow color from the map's <strong>heat ramp</strong> — correct on "
            "dark basemaps, whose additive (screen-blend) ramps are vivid against dark paper. But "
            "the light styles' ramps are engineered for <strong>multiply blending</strong>: they "
            "darken the basemap, so their stops run pale sand → muted brick → near-black "
            "plum/indigo. Painted <em>flat</em> on a thin pin ring (measured live: hottest pin "
            "<code>rgb(92,32,82)</code>, mid ranks <code>rgb(214,152,106)</code>) they read as "
            "greys and tans — the terracotta accent <code>#C0674A</code> appeared nowhere.",
            "Selected pins on light paper pair an ink-black double ring with the paper fill; the "
            "heat tint (26% of an already-pale ramp color) was imperceptible, and a zero-vote "
            "selection has no tint at all — hence “just white bg”.",
        ],
        "fixes": [
            "<strong>Accent-anchored pin ramp on light basemaps.</strong> New "
            "<code>buildPinRampStops(style)</code>: dark basemaps keep the heat ramp verbatim; "
            "light basemaps interpolate pale-accent-tint (lowest rank) → <em>pure accent</em> "
            "(hottest pin, at HEAT_PEAK_POS) → ink-deepened accent (unused tip). Rank still "
            "reads as color depth, and every pin wears the theme. Both pin sites (PBTP squares + "
            "RBTP diamonds) switched; the canvas heatmap keeps the true multiply ramp.",
            "<strong>Guaranteed accent tint on selected fills (light only).</strong> "
        ...
        "files": [
            ...
            "client-react/src/components/GraphLayer/GraphLayer.tsx — both pin ramp sites → buildPinRampStops",
            "client-react/src/styles/globals.css — light-basemap selected-fill baseline accent tint",
        ],
    },
]
...
VERIFY = [
    ...
    "Live (new-york-buses, terracotta): hottest pin now strokes <code>rgb(192,103,74)</code> (= the exact #C0674A accent), lower ranks pale terracotta tints; selected diamond keeps its accent glow + visibly warm fill under the ink ring (verified via computed styles + screenshots).",
    "Live (nyc-bikes, dark): pins unchanged — still the gold/green heat-ramp colors.",
]
...
FILE_CONTEXT = {
    "client-react/src/mapStyles.ts": {
        ...
        "module": ("React client · src/", "map visual identity: basemap tiles, accent, heat ramps, ramp sampling"),
        "file": ("mapStyles.ts", "~350 LOC — MapStyle registry + heat-ramp build/sample helpers shared by heatmap, legend and pins"),
        "outline": [
            ("MapStyle registry", "dark/light styles: accent, tiles, heat ramps", False),
            ("heatTip / heatGradientCss", "incandescent tip + legend gradient", False),
            ("parseRgb / parseColor / mixRgb", "NEW: hex-capable parse + mix helpers", True),
            ("buildHeatRampStops", "the true heat ramp (canvas + legend)", False),
            ("buildPinRampStops", "NEW — pin ramp: heat ramp on dark, accent-anchored on light", True),
            ("sampleHeatRamp", "linear interpolation", False),
        ],
        "blocks": [
            "parseColor() — #rrggbb or rgb() → components (accents are hex, ramps are rgb())",
            "mixRgb() — linear color mix",
            "buildPinRampStops() — light: pale accent tint → pure accent @ HEAT_PEAK_POS → ink-deepened tip",
        ],
    },
    "client-react/src/components/GraphLayer/GraphLayer.tsx": {
        ...
        "file": ("GraphLayer.tsx", "~5000 LOC — the pin ramp is built at two marker sites (PBTP squares, RBTP diamonds)"),
        "outline": [
            ("redraw — heat passes", "canvas heatmap — still buildHeatRampStops (unchanged)", False),
            ("indicatorMarkers (PBTP squares)", "rank → heat → ramp sample → buildPinRampStops", True),
            ("route proposal diamonds (RBTP)", "same rank treatment → buildPinRampStops", True),
        ],
        "blocks": [
            "import buildPinRampStops",
            "indicatorMarkers rampStops — buildHeatRampStops(mapStyle.heat, basemap) → buildPinRampStops(mapStyle)",
            "RBTP diamond rampStops — same switch",
        ],
    },
    "client-react/src/styles/globals.css": {
        "on": ["React / Leaflet client"],
        "module": ("React client · styles/", "design tokens + all component styling"),
        "file": ("globals.css", "~2000 LOC — the .vote-type-indicator pin system lives here"),
        "outline": [
            ("Design tokens", "palette, [data-basemap=light] overrides", False),
            (".vote-type-indicator — glow/fill/outer/inner", "heat glow ring + paper fill", False),
            ("Selected / start / end double ring", "NEW light-mode selected fill: baseline accent tint", True),
        ],
        "blocks": [
            "[data-basemap=light] .is-selected .vote-type-indicator-fill — color-mix 12% + heat·26% of var(--heat-color, var(--accent))",
        ],
    },
}
...
<div class="wrap">
  ...
  <p class="lede">On light map styles the top-proposal pins painted the multiply heat ramp’s pale-sand-to-near-black colors flat on their glow rings — reading as greys — and a selected pin collapsed to a white box with an ink ring. Pins on light basemaps now sample an accent-anchored ramp (hottest pin = the exact theme accent) and selected fills carry a guaranteed accent tint.</p>
  ...
  <footer>
    Generated from <code>changelog/changes.diff</code> by <code>changelog/build_pin_theme_report.py</code>.
  </footer>
</div>
</body>
</html>
