...
# True 30° isometric, and it stays that way.
#
# The QR city (tools/stickers/isoqr.py) squeezes this to ~0.517 so its grid comes
# out a square diamond, and matching it here was tried — the front and back do
# read as one world at the same foreshortening. It was reverted anyway, because
# it costs the wordmark. The letters stand in the wall plane, where +x maps to
# (K, ½), so shrinking K steepens their baseline until CITY EDIT starts to look
# like it is falling over; even the halfway 0.70 leaned more than this reads.
#
# The letters being upright and legible is the whole reason they were stood up
# rather than laid on the roofs. That wins over matching the back.
