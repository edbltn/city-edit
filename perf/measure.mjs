 ...
 *   - loadToHeatmapMs   : navigation → first block-heat apply (the client's
 *                         `first-heat-apply` perf mark, see instrument.js)
 *   - pan: avgFps, p95FrameMs, maxFrameMs, jankFrames (>33ms) during drags
 *   - panRedrawDelayMs  : mouseup → heatmap redraw (canvas builds only; null
 *                         on the feature-state renderer — canvasOps stays empty)
 *   - zoomBlackoutMs    : heatmap clear at zoomstart → redraw after zoomend
 *                         (canvas builds only, same caveat)
 *   - zoom: frame stats during the zoom animation windows
 *
 * Usage:
 *   node measure.mjs --url http://localhost:3000 --label baseline \
 *        [--map nyc-walkways] [--reps 3] [--out results/baseline.json] \
 *        [--view lat,lng,z]
 *
 * Requires the Vite dev server (window.__lmap is only exposed in dev builds).
 */
...
async function main() {
  ...
  const url = `${URL_BASE}/m/${MAP}?lat=${LAT}&lng=${LNG}&z=${Z}`;
  console.log(`[${LABEL}] loading ${url}`);
  await page.goto(url, { waitUntil: "domcontentloaded" });

  // Wait for the first heat apply (topology + votes loaded and painted).
