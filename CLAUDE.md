...
- **Deploy STAGING-FIRST**: push each new image digest to the staging service (`ce-stg-<token>`, `APP_ENV=staging`, own Redis + `votes_staging` DB), verify there, then promote the *same digest* to `desire-path-mapper`. The staging URL is a secret — never commit or paste it into committed files; look it up via `terraform output -raw staging_url`. Data refresh: `make stage-refresh`. Full workflow: [docs/gcp-deployment.md#staging-deploy-here-first](docs/gcp-deployment.md#staging-deploy-here-first).
