...
- **Deploy straight to prod (staging decommissioned 2026-08-01)**: build the image, then push the digest to `desire-path-mapper` — after the mandatory DB backup. There is no staging environment anymore (torn down for lack of traffic; `terraform/staging.tf` lives in git history if it ever needs resurrecting). Verify on prod after deploy: served-asset hash + `curl https://cityedit.org/api/maps`. Full workflow: [docs/gcp-deployment.md#deploying-changes](docs/gcp-deployment.md#deploying-changes).
