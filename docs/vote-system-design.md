# Vote system design — edge storage, block display

> **Status:** living design doc. Covers (1) the existing edge-level vote system as
> it is today, and (2) the new **block-level display** layer being added for
> block voting. The implementation of the new layer lives in
> **`server/block_votes.py`**; this doc is the spec it follows.
>
> One-line summary of the new feature: **votes are stored per graph edge
> (unchanged, backwards-compatible) but displayed per block, deduplicated so each
> `(block, vote_type, user)` counts at most once** — even when a single route laid
> the user's vote across many edges inside one block.
...
intersections; see `streetscape_blocks/`). The display dedup grain is

> **one count per `(map_slug, mode, block_id, vote_type_id, direction, device_id)`**

i.e. **a user's vote_type counts once per block**, regardless of how many edges in
that block carry their vote. Routing, selection, and the write path stay
edge-level and unchanged; blocks are a *display projection* of edge votes.
...
`block_votes.apply_block_delta(...)` is called for each **changed** edge in the
same `voter_lock` + pipeline as `apply_directional`, so edge and block state move
atomically per voter:
...
- **No change** to `edge_votes` (Postgres), `ev:<slug>` (Redis), the write path, or
  the existing `edge_votes[]`/`node_votes[]` arrays. The block layer is **purely
  additive derived state**.
...
| **Block dedup (this design) + rebuild** | **`server/block_votes.py`** |
