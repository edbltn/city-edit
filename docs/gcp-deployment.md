...
> Staging was decommissioned 2026-08-01 (not enough traffic to justify the
> parity copy — see git history for `terraform/staging.tf` /
> `docs/staging-parity-plan.md` if it ever needs resurrecting). Deploys go
> straight to prod: **backup first**, deploy by digest, then verify.
...
### Digest promotion workflow (code-only overlay deploys)

```bash
# 1. Build the overlay image (~2-3 min) → note the digest D
gcloud builds submit --config=cloudbuild.overlay.yaml ...

# 2. BACKUP PROD DB (mandatory), then deploy the digest to prod
...
# 3. Served-asset-hash check on prod
```
